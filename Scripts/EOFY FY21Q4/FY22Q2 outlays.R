...
data <- "~/Data/FY22Q2/Budget/Phase 1 Outlay Workbooks/Kenya_USAID_EOFY_FY 2021.xlsx" 

library(readxl)
df_outlays_fy22q2 <- read_excel("~/Data/FY22Q2/Budget/FY22Q2 Mechanism Outlay Report_Final_Internal_V4.xlsx", 
                                 sheet = "Bilateral Mechanisms", skip = 5)%>%
  clean_names()%>%
  dplyr::rename(operating_unit="ou_for_oha_q2_data_review")

df_outlays_fy22q2<-df_outlays_fy22q2%>%
  select(operating_unit,approved_cop21_planning_level,total_outlays)%>%
   group_by(operating_unit)%>%
  summarise_at(vars(approved_cop21_planning_level,total_outlays,), sum, na.rm = TRUE)%>%
  mutate(budget_execution=percent_clean(total_outlays,approved_cop21_planning_level))%>%
  
  filter(operating_unit!="NA")

##Google sheet output====
target_location <- gs4_get("https://docs.google.com/spreadsheets/d/1F4GfFII4eRYu9ekfkbg0CJw4-SJ-GacaKijPAiW98Fk/edit#gid=0")
sheet_write(df_outlays_fy22q2, target_location, sheet = "OU outlays")
