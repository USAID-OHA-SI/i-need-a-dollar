...
library(glue)
  ...
  # Set paths  
save_dir <- "C:/Users/jmontespenaloza/Documents/GitHub/i-need-a-dollar/images2"
...
dir.create(OU, showWarning=F)
  ...
  # Functions  
    ...
    budget<-list.files("C:/Users/jmontespenaloza/Documents/Bilateral Workbooks for Visuals",full.names = TRUE) #can try and automate via google folder
    glimpse(budget) 
...
# LOAD DATA ============================================================================  
    ...
    #  
     #function build
    #df<-read_xlsx("C:/Users/jmontespenaloza/Documents/Bilat_wcf/Kenya_Bilateral_WCF.xlsx", 
                  #sheet = "OU")
     get_ou_pipeline_bilat<-function(df){
       ...
       #df <- df %>% 
        # pivot_wider(names_from = Attribute,
                    # values_from = data_entry)
       
       df<-t(df)%>%as.data.frame()
       #filters_new <- filters[1:length(df)]
       colnames(df) <- filters
       df=df[-1,]
       ...
       df<-df%>%
         clean_names()%>%
         mutate(across(-operating_unit, as.numeric))%>%
         #mutate(across(-operating_unit,~replace_na(.,0))) %>%
         #mutate_at(vars(2:17),funs(round(.,0))) %>% 
...
# MUNGE ============================================================================
    ...
    df_ou_pipeline2 <- bind_rows(df_ou_pipeline, df_ou)
    df_ou_pipeline2 <- df_ou_pipeline2 %>% group_by(operating_unit)

   
  df_ou_pipeline2 <- df_ou_pipeline2 %>% group_by(operating_unit) %>% 
     summarise(across(),na.rm=TRUE)
  
  write_csv(df_ou_pipeline,"C:/Users/jmontespenaloza/Documents/alicia3.csv")
    df_ou_wcf <- read.csv("C:/Users/jmontespenaloza/Documents/alicia.csv") 
...
# VIZ ============================================================================
    ...
    df_excess<-df_ou_wcf%>%
      #adorn_totals("row",,,, -funding_agency,-operating_unit)%>%
      adorn_totals("row",,,, -operating_unit)%>%
    ...
    df_other_ulo_op_top_10<-df_other_ulo_op_top_10%>%
      ...
      tab_header(
        title = glue::glue(" COP21 USAID End of Fiscal Year Financial Performance Summary"),
        subtitle = "Top 5 OUs with Non-WCF ULOs that have been spent by IPs but not liquidated in Phoenix")%>%
      #gt::tab_source_note(
      # source_note = ("USAID mechanisms only. Partner designations provided by the OHA Local Partners Team. Visual excludes TBDs"))%>%
      gt::tab_source_note(
        source_note = gt::md(glue::glue("**Source**: EOFY Workbooks | ref: {ref_id}"))
  ...
  #Expired awards pipeline graphic
    ...
    df_untouch<-df_ou_pipeline%>%
      adorn_totals("row",,,,-operating_unit)%>%
      arrange(desc(codb_untouchable_pipeline)) %>%
      slice_max(codb_untouchable_pipeline,n=11)%>%
      select(operating_unit,codb_untouchable_pipeline)
    
    df_untouch<- df_untouch%>%
      gt()%>%
      fmt_currency( # add dolar signs
        columns = c(codb_untouchable_pipeline),
        decimals = 0,
        currency = "USD")%>%
      tab_options(
        table.font.names = "Source Sans Pro"
      ...
      cols_label(
        operating_unit = "Operating Unit",
        codb_untouchable_pipeline="CODB Total Untouchable Pipeline",
        #pipeline_in_expired_awards="Expired Awards Pipeline"
      )%>%
      gt::tab_options(
        source_notes.font.size = 8,
      ...
      tab_header(
        title = glue::glue(" COP21 USAID End of Fiscal Year Financial Performance Summary"),
        subtitle = "Top 10 OUs with largest CODB untouchable pipeline")%>%
      #gt::tab_source_note(
      # source_note = ("USAID mechanisms only. Partner designations provided by the OHA Local Partners Team. Visual excludes TBDs"))%>%
      gt::tab_source_note(
        source_note = gt::md(glue::glue("**Source**: EOFY Workbooks | ref: {ref_id}"))
